...
Begin
	Select @time_now = (select Convert(char(8),CURRENT_TIMESTAMP, 8))
	print @time_now + ': Error: must specify an operation.' 
    set @LogMessage = 'Error: must specify an operation.'
    exec SF_Logger @SPName, N'Message', @LogMessage
    GOTO ERR_HANDLER
End
...
if @use_remote = 0
Begin
	set @Command = @ProgDir + 'DBAmpAZ.exe ' + @operation
...
Begin
	...
	set @sql = 'Insert into #ErrorLog Exec ' + @table_server + '.[CData].[Salesforce].InvokeDBAmpAZ ''' + @operation + ''', ''' + @table_name + ''', ''' + @table_server + ''', ''' + @options + ''', ''' + @server + ''''
