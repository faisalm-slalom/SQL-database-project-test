...
Begin
	...
	Select @time_now = (select Convert(char(8),CURRENT_TIMESTAMP, 8))
	print @time_now + ': Running SF_ReplicateAll in remote mode.' 
	set @LogMessage = 'Running SF_ReplicateAll in remote mode.'
	exec SF_Logger @SPName, N'Message', @LogMessage
End
Else
Begin
...
begin
   ...
   begin try
		exec SF_Replicate @table_server, @tn, @options, @use_remote
   end try
   begin catch
