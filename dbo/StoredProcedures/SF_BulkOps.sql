...
Begin
	...
	Select @time_now = (select Convert(char(8),CURRENT_TIMESTAMP, 8))
	print @time_now + ': Running SF_BulkOps in remote mode.' 
	set @LogMessage = 'Running SF_BulkOps in remote mode.'
	exec SF_Logger @SPName, N'Message', @LogMessage
End
Else
Begin
...
Begin
	set @optionsForAZ = SUBSTRING(@operation, CHARINDEX(':', @operation) + 1, LEN(@operation))
...
If @optionsForAZ = ''
Begin
	set @optionsForAZ = 'bulkops,soap'
End
else if @optionsForAZ not like '%bulkapi%'
Begin
	set @optionsForAZ = @optionsForAZ + ',bulkops,soap'
End
Else
Begin
	set @optionsForAZ = @optionsForAZ + ',bulkops'
End
...
Begin
	...
	(Select Cast(Sum(Case When Error not like ' + '''' + '%Operation Successful%' + '''' + ' or Error is null Then 1 Else 0 End) As decimal(18, 3)) As ErrorTotal from ' + @delim_table_name + ')' +
	'/
	(select Cast(Count(*) as decimal(18, 3)) As Total from ' + @delim_table_name + '))'
